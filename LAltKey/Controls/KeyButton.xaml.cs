...
public class KeyButton : System.Windows.Controls.Button
{
    ...
    internal static string DescribeRepeatPolicyForTests(LAltKey.Services.InputMode inputMode, KeyAction? action)
        => ClassifyRepeatPolicy(inputMode, action).ToString();

    internal static bool ShouldCancelCompositionOnRepeatStartForTests(LAltKey.Services.InputMode inputMode, KeyAction? action)
        => ShouldCancelCompositionOnRepeatStart(ClassifyRepeatPolicy(inputMode, action), TryGetVirtualKey(action));

    internal static VirtualKeyCode? GetHoldableVirtualKeyForTests(LAltKey.Services.InputMode inputMode, KeyAction? action)
    {
    ...
    private static RepeatPolicy ClassifyRepeatPolicy(LAltKey.Services.InputMode inputMode, KeyAction? action)
    {
        var vk = TryGetVirtualKey(action);
        if (vk is null)
            return RepeatPolicy.Disabled;

        if (inputMode == LAltKey.Services.InputMode.Unicode)
