...
public class UpdateService
{
    private const string ApiUrl =
        "https://api.github.com/repos/CrowKing63/LAltKey/releases/latest"; // text.
    ...
    public async Task<(bool HasUpdate, string Version, string Url, string InstallerUrl)> CheckAsync()
    {
        try
        {
            ...
            client.DefaultRequestHeaders.UserAgent.ParseAdd("LAltKey");
        ...
        try
        {
            if (root.TryGetProperty("assets", out var assets))
            {
                foreach (var asset in assets.EnumerateArray())
                {
                    ...
                    if (name.StartsWith("LAltKey-Setup-") && name.EndsWith(".exe"))
                    {
