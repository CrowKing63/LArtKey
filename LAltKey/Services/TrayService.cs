...
public class TrayService : IDisposable
{
    ...
    private ContextMenuStrip BuildContextMenu()
    {
        ...
        var versionText = $"LAltKey v{asmVersion?.ToString(3) ?? "0.1.0"}";
    ...
    private async Task CheckForUpdateFromTray()
    {
        try
        {
            ...
            {
                _notifyIcon.ShowBalloonTip(3000, "LAltKey", "Could not read the latest release information.", ToolTipIcon.Warning);
                return;
            }
            ...
            {
                _notifyIcon.ShowBalloonTip(3000, "LAltKey", "LAltKey is up to date.", ToolTipIcon.Info);
            }
        }
        catch (Exception ex)
        {
            _notifyIcon.ShowBalloonTip(3000, "LAltKey", $"Update check failed: {ex.Message}", ToolTipIcon.Error);
        }
    }
    ...
    {
        _notifyIcon.ShowBalloonTip(3000, "LAltKey", message, ToolTipIcon.Info);
    }
